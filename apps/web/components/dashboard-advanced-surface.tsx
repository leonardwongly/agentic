...
type SharedWorkflowMutationState = {
  allowed: boolean;
  reason: string | null;
};
...
type DashboardAdvancedSurfaceProps = {
  ...
  goalBundleById: Map<string, DashboardData["goals"][number]>;
  resolveSharedWorkflowMutationState: (
    workspaceId: string | null | undefined,
    operation: "refine_goal" | "manage_watchers" | "replay_dead_letter_job"
  ) => SharedWorkflowMutationState;
  updateWatcher: (watcherId: string, action: "pause" | "resume") => Promise<void>;
  loadTemplates: () => void;
    ...
    <>
      ...
      >
        ...
        <MemorySearch
          ...
          categories={[...new Set(data.memories.map((memory) => memory.category))]}
          memoryTypes={[...new Set(data.memories.map((memory) => memory.memoryType))]}
          onSelect={() => {}}
        />
        {memoryBulkSelection.selectedIds.size > 0 ? (
          <BulkMemoryActions
            selectedMemories={data.memories
              .filter((memory) => memoryBulkSelection.selectedIds.has(memory.id))
              .map((memory) => ({
                id: memory.id,
                content: memory.content,
                category: memory.category,
        ...
        <div className="list-stack">
          {data.memories.length === 0 ? (
            <NoMemoriesEmpty onAdd={() => document.querySelector<HTMLTextAreaElement>("#section-memory textarea")?.focus()} />
          ) : null}
      ...
      >
        ...
        <div className="list-stack">
          {integrationSurfaces.map(({ integration, readiness }) => {
            ...
            const providerActionLabel = integration.status === "ready" ? "Reconnect Google" : "Connect Google";
              ...
              <div className="list-item vertical" key={integration.id}>
                ...
                <div className="approval-actions">
                  ...
                  {readiness.supportedModes.length > 0 ? (
                    <span className="pill">{readiness.supportedModes.join(" · ")}</span>
                  ) : null}
                  {isManagedGoogle ? (
                    <button type="button" className="secondary-button" onClick={() => void connectGoogleProvider()} disabled={isPending}>
                      {providerActionLabel}
                    </button>
                  ) : (
                    <button
                      type="button"
                      className="secondary-button"
                      onClick={() => void cycleIntegration(integration.id, integration.status)}
                      disabled={isPending}
                    >
                      Toggle
                    </button>
      ...
      >
        ...
        <div className="note-toolbar">
          <input value={noteQuery} onChange={(event) => setNoteQuery(event.target.value)} placeholder="Search local notes" />
          ...
          {templates.map((template) => (
            <div className="list-item vertical" key={template.id}>
              ...
              <div className="goal-item-actions">
                ...
                {template.schedule.enabled ? <span className="pill">{template.schedule.cron}</span> : null}
