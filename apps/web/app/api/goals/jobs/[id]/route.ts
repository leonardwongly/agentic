...
export async function GET(request: Request, context: RouteContext) {
  try {
    ...
    if (!id.trim()) {
      throw new ApiRouteError(400, "Goal job id is required.");
    }
    ...
    if (job.status === "completed") {
      ...
      if (!bundle) {
        throw new Error(`Goal ${job.payload.goalId} is missing after job completion.`);
      }
    ...
    return handleApiError(error, "Failed to load goal job.");
  }
}
