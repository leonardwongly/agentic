...
| Agents create/update/import | JSON body, path param, exported agent payload | agent definition and dashboard state | authenticated API caller to custom-agent boundary | persists agent definitions | route + runner tests | custom side-effect agents are draft-quarantined until explicit owner review confirms activation |
...
| Gmail and Calendar mutations | typed action intent, provider credential, idempotency key, abort signal | provider draft/message/event reference | action execution to Google provider boundary | creates Gmail drafts/messages or Calendar events/updates | adapter + action contract coverage | provider-side search/reconciliation still depends on Google API availability during retries |
...
- Required Google provider mutations to carry stable idempotency keys before outbound calls, added duplicate-safe Gmail draft/send and Calendar event reconciliation, and bounded mutation calls with abort-aware timeout signals.
