...
| --- | --- | --- | --- | --- | --- |
| `send_message` | [`SendMessageActionIntentSchema`](https://github.com/leonardwongly/agentic/blob/main/packages/contracts/src/index.ts) | Gmail draft/send calls in [`executeApprovedTask(...)`](https://github.com/leonardwongly/agentic/blob/main/packages/orchestrator/src/execution-dispatch.ts) | `draft` or `send` | approved typed approval required | communications follow-up and reply drafts |
| `schedule_event` | [`ScheduleEventActionIntentSchema`](https://github.com/leonardwongly/agentic/blob/main/packages/contracts/src/index.ts) | Calendar event creation in [`executeApprovedTask(...)`](https://github.com/leonardwongly/agentic/blob/main/packages/orchestrator/src/execution-dispatch.ts) | `schedule` | approved typed approval required | calendar commitments and handoff scheduling |
| `create_note` | [`CreateNoteActionIntentSchema`](https://github.com/leonardwongly/agentic/blob/main/packages/contracts/src/index.ts) | Local notes creation in [`executeApprovedTask(...)`](https://github.com/leonardwongly/agentic/blob/main/packages/orchestrator/src/execution-dispatch.ts) | `create` | approved typed approval required | workspace note creation and execution artifacts |
| `manual_review` | [`ManualReviewActionIntentSchema`](https://github.com/leonardwongly/agentic/blob/main/packages/contracts/src/index.ts) | no side effect; dispatcher returns `execution.skipped` | none | used when typed payload is absent or not trusted | fail-closed fallback for untyped approvals |
...
- [`packages/orchestrator/src/execution-dispatch.ts`](https://github.com/leonardwongly/agentic/blob/main/packages/orchestrator/src/execution-dispatch.ts)
- [`packages/repository/src/index.ts`](https://github.com/leonardwongly/agentic/blob/main/packages/repository/src/index.ts)
- [`apps/web/app/api/approvals/[id]/respond/route.ts`](https://github.com/leonardwongly/agentic/blob/main/apps/web/app/api/approvals/[id]/respond/route.ts)
...
1. The caller must be authenticated at the route boundary.
   - [`requireApiSession(...)`](https://github.com/leonardwongly/agentic/blob/main/apps/web/app/api/approvals/[id]/respond/route.ts) resolves the principal.
...
2. The approval must be visible to the authenticated user.
   - [`respondToApproval(...)`](https://github.com/leonardwongly/agentic/blob/main/packages/repository/src/index.ts) scopes approvals through goal visibility for the acting user.
...
3. Only an approved approval may authorize typed execution.
   - [`findApprovedApproval(...)`](https://github.com/leonardwongly/agentic/blob/main/packages/orchestrator/src/execution-dispatch.ts) ignores pending and rejected approvals.
...
4. The requested typed action must fit within the task capability grant.
   - [`requiredCapabilitiesForActionIntent(...)`](https://github.com/leonardwongly/agentic/blob/main/packages/orchestrator/src/execution-dispatch.ts) derives the minimum capability set for each supported action family.
   - [`validateTypedActionBoundary(...)`](https://github.com/leonardwongly/agentic/blob/main/packages/orchestrator/src/execution-dispatch.ts) rejects the action if the task grant does not cover those capabilities.
...
6. Approvals without a typed payload must not trigger inferred side effects.
   - [`buildFallbackApprovalActionIntent(...)`](https://github.com/leonardwongly/agentic/blob/main/packages/repository/src/index.ts) converts those approvals into explicit `manual_review`.
...
7. Built-in agents may only emit typed payloads from explicit validated execution cues.
   - [`runAgent(...)`](https://github.com/leonardwongly/agentic/blob/main/packages/agents/src/index.ts) only synthesizes `send_message` and `schedule_event` intents when the normalized request contains explicit labeled fields that pass [`ActionIntentSchema`](https://github.com/leonardwongly/agentic/blob/main/packages/contracts/src/index.ts) validation.
   ...
   - [`processUserRequest(...)`](https://github.com/leonardwongly/agentic/blob/main/packages/orchestrator/src/index.ts) passes the normalized request through to agent execution so this validation runs on the real user request, not only the catalog title.
...
- [`tests/execution-dispatch.test.ts`](https://github.com/leonardwongly/agentic/blob/main/tests/execution-dispatch.test.ts)
...
- [`tests/action-intent-contract.test.ts`](https://github.com/leonardwongly/agentic/blob/main/tests/action-intent-contract.test.ts)
...
- [`tests/agents.test.ts`](https://github.com/leonardwongly/agentic/blob/main/tests/agents.test.ts)
...
- [`tests/route-user-scope.test.ts`](https://github.com/leonardwongly/agentic/blob/main/tests/route-user-scope.test.ts)
...
- [`tests/nl-intent-route.test.ts`](https://github.com/leonardwongly/agentic/blob/main/tests/nl-intent-route.test.ts)
