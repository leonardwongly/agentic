...
| --- | --- | --- | --- | --- | --- |
| `send_message` | [`SendMessageActionIntentSchema`](packages/contracts/src/index.ts) | Gmail draft/send calls in [`executeApprovedTask(...)`](packages/orchestrator/src/execution-dispatch.ts) | `draft` or `send` | approved typed approval required | communications follow-up and reply drafts |
| `schedule_event` | [`ScheduleEventActionIntentSchema`](packages/contracts/src/index.ts) | Calendar event creation in [`executeApprovedTask(...)`](packages/orchestrator/src/execution-dispatch.ts) | `schedule` | approved typed approval required | calendar commitments and handoff scheduling |
| `create_note` | [`CreateNoteActionIntentSchema`](packages/contracts/src/index.ts) | Local notes creation in [`executeApprovedTask(...)`](packages/orchestrator/src/execution-dispatch.ts) | `create` | approved typed approval required | workspace note creation and execution artifacts |
| `manual_review` | [`ManualReviewActionIntentSchema`](packages/contracts/src/index.ts) | no side effect; dispatcher returns `execution.skipped` | none | used when typed payload is absent or not trusted | fail-closed fallback for untyped approvals |
...
- [`packages/orchestrator/src/execution-dispatch.ts`](packages/orchestrator/src/execution-dispatch.ts)
- [`packages/repository/src/index.ts`](packages/repository/src/index.ts)
- [`apps/web/app/api/approvals/[id]/respond/route.ts`](apps/web/app/api/approvals/[id]/respond/route.ts)
...
1. The caller must be authenticated at the route boundary.
   - [`requireApiSession(...)`](apps/web/app/api/approvals/[id]/respond/route.ts) resolves the principal.
...
2. The approval must be visible to the authenticated user.
   - [`respondToApproval(...)`](packages/repository/src/index.ts) scopes approvals through goal visibility for the acting user.
...
3. Only an approved approval may authorize typed execution.
   - [`findApprovedApproval(...)`](packages/orchestrator/src/execution-dispatch.ts) ignores pending and rejected approvals.
...
4. The requested typed action must fit within the task capability grant.
   - [`requiredCapabilitiesForActionIntent(...)`](packages/orchestrator/src/execution-dispatch.ts) derives the minimum capability set for each supported action family.
   - [`validateTypedActionBoundary(...)`](packages/orchestrator/src/execution-dispatch.ts) rejects the action if the task grant does not cover those capabilities.
...
6. Approvals without a typed payload must not trigger inferred side effects.
   - [`buildFallbackApprovalActionIntent(...)`](packages/repository/src/index.ts) converts those approvals into explicit `manual_review`.
...
7. Built-in agents may only emit typed payloads from explicit validated execution cues.
   - [`runAgent(...)`](packages/agents/src/index.ts) only synthesizes `send_message` and `schedule_event` intents when the normalized request contains explicit labeled fields that pass [`ActionIntentSchema`](packages/contracts/src/index.ts) validation.
   ...
   - [`processUserRequest(...)`](packages/orchestrator/src/index.ts) passes the normalized request through to agent execution so this validation runs on the real user request, not only the catalog title.
...
- [`tests/execution-dispatch.test.ts`](tests/execution-dispatch.test.ts)
...
- [`tests/action-intent-contract.test.ts`](tests/action-intent-contract.test.ts)
...
- [`tests/agents.test.ts`](tests/agents.test.ts)
...
- [`tests/repository.test.ts`](tests/repository.test.ts)
...
- [`tests/route-user-scope.test.ts`](tests/route-user-scope.test.ts)
...
- [`tests/nl-intent-route.test.ts`](tests/nl-intent-route.test.ts)
