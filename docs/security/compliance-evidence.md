...
- Keep uploaded GitHub Actions evidence artifacts to 30 days unless an investigation requires manual preservation. The scheduled `Artifact Cleanup` workflow deletes older artifacts daily, and manual runs can use `dry-run` to preview deletions before changing repository state.
