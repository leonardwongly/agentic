...
Phase 2 privacy work now keeps a versioned data-handling registry in [config/privacy/data-controls.json](https://github.com/leonardwongly/agentic/blob/main/config/privacy/data-controls.json). The registry exists so minimization, masking, tokenization, retention, and lifecycle coverage stay explicit instead of being inferred from scattered route logic.
