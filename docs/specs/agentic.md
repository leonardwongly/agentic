...
| `/api/ready` | `GET` | Return unauthenticated public-safe cached readiness summary and the detailed readiness endpoint path |
| `/api/ready/details` | `GET` | Return authenticated fresh detailed readiness state for access key, storage, auth runtime state, request identity, async execution, and connector health |
