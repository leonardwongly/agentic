...
describe("release closeout evidence", () => {
  ...
  it("renders a reviewer summary for valid manifests", () => {
    ...
    expect(rendered).toContain("- Pull requests: 40");
    ...
    expect(manifest.pullRequests).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          number: 914,
          status: "merged",
          url: "https://github.com/leonardwongly/agentic/pull/914"
        })
      ])
    );
  });

  it("keeps the latest GitHub sync evidence safety hardening PRs in the closeout package", () => {
    const manifest = readCheckedInManifest();
    ...
    expect(manifest.pullRequests).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          number: 915,
          status: "merged",
          url: "https://github.com/leonardwongly/agentic/pull/915"
        }),
        expect.objectContaining({
          number: 916,
          status: "merged",
    ...
    expect(manifest.pullRequests).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          number: 929,
          status: "merged",
          url: "https://github.com/leonardwongly/agentic/pull/929"
        })
      ])
    );
    expect(localCiGate?.evidence).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          kind: "github_action",
          ref: "https://github.com/leonardwongly/agentic/actions/runs/26253307486",
          status: "passed"
        })
      ])
    );
