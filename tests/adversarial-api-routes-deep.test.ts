...
// Import after mocks
import { POST as sessionPost, DELETE as sessionDelete } from "../apps/web/app/api/session/route";
...
import { GET as goalsGet, POST as goalsPost } from "../apps/web/app/api/goals/route";
import {
...
import { createRouteTestRepository, expectBaseSecurityHeaders } from "./route-test-helpers";
...
describe("Adversarial API Route Security Tests", () => {
  ...
  const originalAllowedRepos = process.env.AGENTIC_GITHUB_ISSUE_ALLOWED_REPOSITORIES;
  const originalMachineTokens = process.env.AGENTIC_MACHINE_TOKENS_JSON;

  beforeEach(async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "agentic-adversarial-"));
  ...
  afterEach(() => {
    ...
    process.env.AGENTIC_GITHUB_ISSUE_ALLOWED_REPOSITORIES = originalAllowedRepos;
  ...
  describe("Input Validation", () => {
    ...
    it("should reject prototype pollution via __proto__ key", async () => {
      const maliciousPayload = JSON.parse('{"__proto__": {"polluted": true}, "request": "test"}');
  ...
  describe("Rate Limiting", () => {
    it("should enforce rate limits on session creation", async () => {
      ...
      const makeRequest = () => new Request("http://localhost/api/session", {
        method: "POST",
  ...
  describe("Path Traversal and Injection", () => {
    ...
    it("should reject SQL injection patterns in query parameters", async () => {
      ...
      const request = new Request(`http://localhost/api/goals?search=${encodeURIComponent(sqlInjection)}`, {
        method: "GET",
        headers: {
          [AGENTIC_ACCESS_KEY_HEADER]: TEST_ACCESS_KEY
        }
  ...
  describe("Webhook Signature Forgery", () => {
    ...
    it("should reject replayed webhooks with same delivery ID (idempotency)", async () => {
      ...
      const makeRequest = () => new Request("http://localhost/api/github/issues/webhook", {
        method: "POST",
        headers: {
          "content-type": "application/json",
          "x-github-event": "issues",
