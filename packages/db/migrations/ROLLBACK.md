...
- `0009_job_scheduling_controls.sql`: Adds job priority, queue, concurrency, timeout, and scheduling indexes. Roll back by disabling priority/concurrency-aware workers first, then restoring a pre-migration backup if queue metadata must be removed.
- `0010_agent_memory_scope.sql`: Adds agent-scoping metadata and lookup index for memory records. Roll back application code that reads scoped memories first, then restore from a pre-migration backup if scoped metadata must be removed.
- `0011_provider_side_effect_ledger.sql`: Adds the provider side-effect ledger for durable mutation idempotency. Roll back by disabling ledger-aware provider execution first; preserve ledger rows for duplicate-delivery forensics unless an operator-approved restore plan removes them from backup.
- `0012_worker_runtime_health.sql`: Adds the shared worker runtime heartbeat table for split web/worker (for example serverless) deployments. Roll back by unsetting the DB heartbeat path (readiness falls back to durable queue state) and restoring from a pre-migration backup only if the table must be removed; the table holds transient heartbeat state, so dropping it loses no durable work.
- `0013_memory_belief_revision.sql`: Adds memory belief-revision metadata (`version`, `supersedes`, `valid_from`) supporting the stored `contradicted`/`expired` assertion states. The DDL is additive and idempotent (`add column if not exists`). Roll back application code that reads versioning/supersession first; the columns are nullable/defaulted so ignoring them is safe, and restore from a pre-migration backup only if the columns must be removed.
