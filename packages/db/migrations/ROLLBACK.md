...
- `0010_provider_side_effect_ledger.sql`: Adds the provider side-effect ledger for durable mutation idempotency. Roll back by disabling ledger-aware provider execution first; preserve ledger rows for duplicate-delivery forensics unless an operator-approved restore plan removes them from backup.
