...
function buildSpecialistPrompt(task: Task, scenario: string, scaffold: string): string {
  return [
    `You are the ${task.assignedAgent} specialist in a governed execution system.`,
    "Produce the artifact body for the task below. Do not perform or promise any external",
    "action; every side effect stays behind explicit human approval.",
    "",
    `Task: ${task.title}`,
    `Scenario: ${scenario}`,
    `Risk class: ${task.riskClass}`,
    "",
    "Deterministic scaffold to improve on:",
    scaffold,
    "",
    "Return only the improved artifact body as plain text."
  ].join("\n");
}

function buildActionExtractionPrompt(task: Task, scenario: string): string {
  const schema =
    task.assignedAgent === "communications"
      ? "To: <recipient>\nSubject: <subject>\nBody: <message>\nMode: draft"
      : "Event: <title>\nStart: <ISO start>\nEnd: <ISO end>\nAttendees: <comma-separated emails>";
  return [
    `Extract a single ${task.assignedAgent} action from the request, only if one is clearly requested.`,
    "Reply ONLY with these labeled lines, or the single word NONE:",
    "",
    schema,
    "",
    `Request: ${scenario}`
...
// Surfaces a model-proposed action from natural language, but routes it through the same
// deterministic builders the non-model path uses. Those builders enforce the capability
// allowlist and schema validation, so a proposed action can never exceed what the task is
// already authorized for; execution still requires approval downstream.
async function proposeModelActionIntent(task: Task, scenario: string): Promise<ActionIntent | null> {
  const supportsAction =
    (task.assignedAgent === "communications" &&
      (task.toolCapabilities.includes("draft") || task.toolCapabilities.includes("send"))) ||
    (task.assignedAgent === "calendar" && task.toolCapabilities.includes("schedule"));
  if (!supportsAction) {
    return null;
  }

  const fields = await runTextModel({ prompt: buildActionExtractionPrompt(task, scenario), maxTokens: 300 });
  if (!fields) {
    return null;
  }

  return task.assignedAgent === "communications"
    ? maybeBuildCommunicationsActionIntent(task, fields)
    : maybeBuildCalendarActionIntent(task, fields);
}

/**
 * Async, model-backed variant of {@link runAgent}. When a model provider is configured
 * (and outside test runs), the artifact body is generated by the model; otherwise it
 * returns the deterministic baseline unchanged. executionMode and confidence always come
 * from the deterministic runner. A model-proposed actionIntent is accepted only when the
 * deterministic runner produced none, and only through the same capability-gated builders,
 * so the model can surface an action from natural language but cannot escalate capability,
 * escalate risk, or bypass approval. Any model error falls back to the baseline.
 */
export async function runAgentWithModel(
  ...
  const baseArtifact = baseline.artifacts[0];
  if (!baseArtifact) {
    return baseline;
  }
