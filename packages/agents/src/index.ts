 ...
 * returns the deterministic baseline unchanged. Governance-relevant fields (executionMode,
 * actionIntent, confidence) always come from the deterministic runner, so the model can
 * enrich prose but cannot escalate risk or bypass approval. Any model error falls back to
 * the baseline.
 */
  ...
  try {
    ...
    return AgentResultSchema.parse({
      ...
      artifacts: baseline.artifacts.map((artifact, index) =>
        index === 0 ? { ...artifact, content: text.slice(0, 4000) } : artifact
      )
    });
  } catch {
    return baseline;
